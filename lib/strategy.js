        ...
        self._shouldLoadUserProfile(iss, sub, function (err, load) {
          ...
          function onProfileLoaded(profile) {
            ...
            if (self._passReqToCallback) {
              ...
              if (arity == 9) {
                self._verify(req, params, sub, profile, jwtClaims, accessToken, refreshToken, iss, verified);
              } else if (arity == 8) {
                self._verify(req, params, sub, profile, accessToken, refreshToken, ississ, verified);
              } else if (arity == 7) {
                self._verify(req, params, sub, profile, accessToken, refreshToken, verified);
              } else if (arity == 5) {
                self._verify(req, params, sub, profile, verified);
              } else { // arity == 4
                self._verify(req, params, sub, verified);
              }
            } else {
              var arity = self._verify.length;
              if (arity == 8) {
                self._verify(params, sub, profile, jwtClaims, accessToken, refreshToken, iss, verified);
              } else if (arity == 7) {
                self._verify(params, sub, profile, accessToken, refreshToken, iss, verified);
              } else if (arity == 6) {
                self._verify(params, sub, profile, accessToken, refreshToken, verified);
              } else if (arity == 4) {
                self._verify(params, sub, profile, verified);
              } else { // arity == 3
                self._verify(params, sub, verified);
              }
            }
          } // onProfileLoaded    
